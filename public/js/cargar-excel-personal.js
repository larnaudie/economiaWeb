...
const confirmarNuevasCategoriasBtn = document.getElementById("confirmarNuevasCategoriasBtn");
const cancelarNuevasCategoriasBtn = document.getElementById("cancelarNuevasCategoriasBtn");
const selectAllNuevasCategoriasBtn = document.getElementById("selectAllNuevasCategoriasBtn");
const unselectAllNuevasCategoriasBtn = document.getElementById("unselectAllNuevasCategoriasBtn");
...
confirmarNuevasCategoriasBtn.addEventListener("click", confirmarNuevasCategorias);
...
function buscarCategoriaPorNombre(nombre) {
    ...
    return categoriasCache.find((categoria) =>
        normalizarTexto(categoria.nombre) === nombreNormalizado
    );
}
...
async function crearCategoriaSiNoExiste(nombre) {
    ...
    const data = await apiRequest("/categorias", "POST", { nombre: limpio }, token);
        ...
        .map((row, index) => {
            ...
            if (flujoBancario != null && flujoBancario !== 0) {
                if (economiaReal != null) {
                    porcentajeEconomiaReal = Number(((economiaReal / flujoBancario) * 100).toFixed(2));
...
async function confirmarNuevasCategorias() {
    ...
    try {
        const seleccionadas = nuevasCategoriasPendientes.filter(item => item.selected);
        ...
        importedRows.forEach((row) => {
            if (!row.categoria && row.categoriaNombreOriginal) {
                const categoriaEncontrada = buscarCategoriaPorNombre(row.categoriaNombreOriginal);
        ...
        nuevasCategoriasError.textContent = error.message || "Error al crear las nuevas categorías.";
    }
}
...
function renderPreview() {
    ...
    previewBody.innerHTML = importedRows
        .map((row) => {
            ...
            const economiaReadonlyAttr = Number(row.flujoBancario) === 0 ? "" : "readonly";
            const porcentajeDisabledAttr = Number(row.flujoBancario) === 0 ? "disabled" : "";
        ...
        <tr data-id="${row.localId}">
          ...
          <td>
            <input type="date" class="row-fecha" value="${row.fecha || ""}" ${readOnlyAttr}>
          </td>
          ...
          <td>
            <input type="number" class="row-flujo" step="0.01" value="${row.flujoBancario ?? 0}" ${readOnlyAttr}>
          </td>
  ...
  value="${row.economiaReal ?? 0}" ${economiaReadonlyAttr}>
          ...
          <td>
            <div class="inline-group">
              <button type="button" class="edit-row-btn" data-id="${row.localId}" ${row.created ? "disabled" : ""}>
...
function attachRowEvents() {
    ...
    document.querySelectorAll("#previewBody tr").forEach((rowElement) => {
        ...
        rowElement.querySelector(".row-economia")?.addEventListener("input", (e) => {
            ...
            if (flujo === 0) {
                updateRow(localId, "economiaReal", Number.isNaN(nuevoValor) ? 0 : nuevoValor);
                updateRow(localId, "porcentajeEconomiaReal", 0);
...
async function handleConfirmRow(event) {
    ...
    try {
        ...
        if (!categoriaId) {
            const categoriaCreada = await crearCategoriaSiNoExiste(row.categoriaNombreOriginal);
